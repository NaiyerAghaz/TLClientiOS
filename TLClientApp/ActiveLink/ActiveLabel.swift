...
@IBDesignable open class ActiveLabel: UILabel {
    ...
    open var enabledTypes: [ActiveType] = [.mention, .hashtag, .url, .email,.phone]
    ...
    open func handlePhoneTap(_ handler: @escaping (String) -> ()) {
    ...
    func onTouch(_ touch: UITouch) -> Bool {
        ...
        case .ended, .regionExited:
            ...
            case .phone(let element):didTapStringPhone(element)
    ...
    fileprivate func updateAttributesWhenSelected(_ isSelected: Bool) {
        ...
        if isSelected {
            ...
            case .phone: selectedColor = URLSelectedColor ?? URLColor
            }
            ...
            case .phone: unselectedColor = URLColor
    ...
    fileprivate func didTapStringPhone(_ stringPhone: String) {
        guard let phoneHandler = phoneTapHandler else {
            delegate?.didSelect(stringPhone, type: .phone)
            return
        }
        phoneHandler(stringPhone)
    }
