...
class BlockedAppointmentDetailVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        appointmentDetailTV.rowHeight = UITableView.automaticDimension
        appointmentDetailTV.estimatedRowHeight = UITableView.automaticDimension
        //        appointmentDetailTV.tableFooterView = UIView(frame: .zero)
        //        appointmentDetailTV.rowHeight = UITableView.automaticDimension
        //        appointmentDetailTV.estimatedRowHeight = 600
                        ...
                        do {
                            ...
                            do {
                                ...
                                if let jsonArray = try JSONSerialization.jsonObject(with: data, options : .allowFragments) as? [Dictionary<String,Any>]
                                {
                                    ...
                                    self.appointmentDataArray[0].collapsedStatus = true
                                    print("APPOINTMENT DATA ARRAY IS \(self.appointmentDataArray)")
                                    getAppointmentStatus(appointmentType: (appointmentDataArray[0].AppointmentStatusType)!) { headingStr, statusStr, colorStr in
                                            self.lblHeaderStatus.text = headingStr
                                           
                                            self.lblHeaderStatus.backgroundColor = hexStringToUIColor(hex: colorStr!)
...
extension BlockedAppointmentDetailVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let blockedModel = BlockedVModel()
        ...
        getAppointmentStatus(appointmentType: (index.AppointmentStatusType)!) { headingStr, statusStr, colorStr in
          //  self.appointStatusHeadingLbl.text = headingStr
           ...
           // self.appointStatusHeadingLbl.backgroundColor = hexStringToUIColor(hex: colorStr!)
            cell.statusLbl.textColor = hexStringToUIColor(hex: colorStr!)
        }
        
       // cell.statusLbl.text =  "\(index.AppointmentStatusType ?? "N/A")"
       // let totalAddress = "\(index?.VenueAddress ?? ""),\(index.c ?? ""),\(showAppointmentArr?.stateName ?? ""),\(showAppointmentArr?.zipcode ?? "")"
       ...
       // let appointmentType = index.AppointmentType ?? "N/A"
        if index.JobType == "Virtual Meeting" {
            cell.NoteText.text = "Notes"
            cell.locationText.text = "Virtual Meeting Details Links, ID and Pass Codes"
            cell.venueAddressStack.visibility = .gone
            cell.departmentStack.visibility = .gone
            cell.venueNameStack.visibility = .gone
        }
        else if index.JobType == "Telephone Conference"{
            cell.NoteText.text = "Conference Call Details-Phone Number"
            cell.locationText.text = "Details Links, ID and Pass Codes"
            cell.venueAddressStack.visibility = .gone
            cell.departmentStack.visibility = .gone
            cell.venueNameStack.visibility = .gone
        }
        else {
            cell.venueAddressStack.visibility = .visible
            cell.departmentStack.visibility = .visible
            cell.venueNameStack.visibility = .visible
        }
        ...
        cell.lblSpeciality.text = index.SpecialityName != nil ? index.SpecialityName : "N/A"
        
        
        cell.lblLocation.text = (index.CLocation != "") ? index.CLocation : "N/A"
        cell.lblLocation.handleURLTap { url in
            
            CEnumClass.share.activeLinkCall(activeURL: url)
        }
        
        if index.CaseNumber != "" && index.CaseNumber != nil {
            blockedModel.hitApiDecryptValue(value: index.CaseNumber!) { apt, code in
                cell.lblClientpatientRefrence.text = code
            }
        ...
        if index.ClientName != "" && index.ClientName != nil {
            blockedModel.hitApiDecryptValue(value: index.ClientName!) { apt, code in
                cell.lblClientPatientName.text = code
            }
            
        }
        if index.ClientCase != "" && index.ClientCase != nil {
            blockedModel.hitApiDecryptValue(value: index.ClientCase!) { apt, code in
                cell.patientintialsLbl.text = code
            }
            
        }
        cell.lblNotes.text = index.CText != "" ? index.CText : "N/A"
        cell.lblNotes.handleURLTap { url in
            
            CEnumClass.share.activeLinkCall(activeURL: url)
        }
        /*if ((CustomerCancelRequest != null && CustomerCancelRequest.equalsIgnoreCase("0")) || (data.getString("AppointmentStatusTypeID") == "3" || data.getString("AppointmentStatusTypeID") == "5" || data.getString("AppointmentStatusTypeID") == "13" || data.getString("AppointmentStatusTypeID") == "7" || data.getString("AppointmentStatusTypeID") == "8" || data.getString("AppointmentStatusTypeID") == "4")) {
...
//        })
        ...
        if index.AppointmentType == "Schedule OPI" || index.AppointmentType == "Schedule VRI" {
            cell.lblInterpereterName.attributedText =  index.Interpretername?.convertHtmlToAttributedStringWithCSS(font: UIFont.systemFont(ofSize: 10), csscolor: "black", lineheight: 5, csstextalign: "left")
        }else {
            cell.lblInterpereterName.text = index.Interpretername != "" ? index.Interpretername : "Waiting on vendor name"
...
class BlockedAppointmentDetailTVC: UITableViewCell {
    ...
    //Hiding parts on TC, VC
    
    @IBOutlet weak var lblLocation: ActiveLabel!
    
    @IBOutlet weak var venueNameStack: UIStackView!
    @IBOutlet weak var venueAddressStack: UIStackView!
    
    @IBOutlet weak var departmentStack: UIStackView!
    
    @IBOutlet weak var NoteText: UILabel!
    @IBOutlet weak var locationText: UILabel!
    @IBOutlet weak var lblNotes: ActiveLabel!
    @IBOutlet weak var lblSpeciality: UILabel!
    @IBOutlet weak var lblClientPatientName: UILabel!
    @IBOutlet weak var lblInterpereterName: UILabel!
    @IBOutlet weak var lblClientpatientRefrence: UILabel!
