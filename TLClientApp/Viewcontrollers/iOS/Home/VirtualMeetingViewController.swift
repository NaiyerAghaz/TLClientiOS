//
//  VirtualMeetingViewController.swift
//  TLClientApp
//
//  Created by SMIT 005 on 07/12/21.
//
...
import  Alamofire
import iOSDropDown
class VirtualMeetingViewController: UIViewController, UITextFieldDelegate {
...
//        let newDate = endDate.nearestHour() ?? endDate
//        let showEndDate = dateFormatter.string(from: newDate)
        let dateFormatter = DateFormatter()
//        MM/dd/yyyy
        ...
        startTimeTF.text = dateFormatterTime.string(from: currentDateTime)
        
        let endTimee = Date().adding(minutes: 60).nearestHour() ?? Date()
        endDateTF.text = dateFormatterDate.string(from: endTimee)
        endTimeTF.text = dateFormatterTime.string(from: endTimee)
...
//        self.loadedONTF.text = startDate
        ...
        dateFormatterrr.dateFormat = "MM/dd/yyyy h:mm a"
                let startDatee =  dateFormatterrr.string(from: Date().nearestHour() ?? Date ())
                 let endDatee = Date().adding(minutes: 120)
                let newEndDatee = endDatee.nearestHour() ?? Date ()
                let showEndDatee = dateFormatterrr.string(from: newEndDatee)
    ...
    @IBAction func actionSelectStartDateonly(_ sender: UIButton) {
    ...
    @IBAction func actionSelectEndTimeOnly(_ sender: UIButton) {
        let minDate = Date().adding(minutes: 60)
...
//                        let roundOff = selectedDate.nearestHour() ?? selectedDate
                        self?.startDateTF.text = selectedDate.dateString("MM/dd/YYYY")
                         let endDate = selectedDate.adding(minutes: 120)
