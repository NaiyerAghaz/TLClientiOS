...
/*class OnsiteInterpretationNewViewController: UIViewController , UITextFieldDelegate{
    ...
    @IBOutlet weak var blockedAppointmentView: UIView!
    ...
    @IBOutlet weak var recurringAppointmentView: UIView!
    @IBOutlet weak var departmentOptionMajorView: UIView!
    @IBOutlet weak var activateDeactivateView: UIView!
    
    @IBOutlet weak var blockedLanguageTF: iOSDropDown!
    
    @IBOutlet weak var blockedGenderTF: iOSDropDown!
    
    
    @IBOutlet weak var blockedEndTimeTF: UITextField!
    @IBOutlet weak var blockedAptStartTimeTF: UITextField!
    @IBOutlet weak var blockedAptDateTF: UITextField!
    @IBOutlet weak var regularMainDetailSectionview: UIView!
    @IBOutlet weak var regularMainAppointmentView: UIView!
    @IBOutlet weak var regularMainVenueDetailView: UIView!
    ...
    override func viewDidLoad() {
        ...
        self.recurringAppointmentView.isHidden = true
        self.blockedAppointmentView.isHidden = true
        ...
        self.blockedAptDateTF.text = dateFormatterDate.string(from: currentDateTime)
        self.blockedEndTimeTF.text = dateFormatterTime.string(from: endTimee)
        self.blockedAptStartTimeTF.text = dateFormatterTime.string(from: currentDateTime)
   ...
   //MARK: - Blocked Date and Time Action
    
    
    @IBAction func actionBlockedEndTime(_ sender: UIButton) {
        let minDate = self.selectedStartTimeForPicker.adding(minutes: 120)//Date().adding(minutes: 120)
        RPicker.selectDate(title: "Select End Time", cancelText: "Cancel", datePickerMode: .time, selectedDate: selectedEndTimeForPicker,minDate: minDate, maxDate: Date().dateByAddingYears(5), didSelectDate: {[weak self] (selectedDate) in
            ...
            self?.blockedEndTimeTF.text = roundoff.dateString("hh:mm a")
        })
    }
    @IBAction func blockedStartTimeSelect(_ sender: UIButton) {
        ...
        RPicker.selectDate(title: "Select Start Time", cancelText: "Cancel", datePickerMode: .time, selectedDate: selectedStartTimeForPicker, minDate: minDate, maxDate: Date().dateByAddingYears(5), didSelectDate: {[weak self] (selectedDate) in
            ...
            self?.blockedAptStartTimeTF.text = roundoff.dateString("hh:mm a")
             
            self?.blockedEndTimeTF.text = endTimee.dateString("hh:mm a")
        })
        
    }
    @IBAction func actionBlockedAppointmentDate(_ sender: UIButton) {
        ...
        RPicker.selectDate(title: "Select Date & Time", cancelText: "Cancel", datePickerMode: .date, minDate: Date(), maxDate: Date().dateByAddingYears(5), didSelectDate: {[weak self] (selectedDate) in
            ...
            let  roundoff = selectedDate//.nearestHour() ?? selectedDate
            self?.blockedAptDateTF.text = roundoff.dateString("MM/dd/YYYY")
...
//        }
    ...
    @IBAction func actionAddBlockedApt(_ sender: UIButton) {
        ...
        if self.blockedAppointmentArr.count < 4 {
            if blockedAppointmentArr.allSatisfy({ BlockedAppointmentData in
                ...
                let itemA = BlockedAppointmentData(AppointmentDate: dateFormatterDate.string(from: currentDateTime), startTime: dateFormatterTime.string(from: currentDateTime), endTime: dateFormatterTime.string(from: endTimee), languageID: Int(self.languageID) ?? 0, genderID: "", clientName: "", ClientIntials: "", ClientRefrence: "", venueID: "", DepartmentID: 0, contactID: 0, location: "", SpecialNotes: "", rowIndex: blockedAppointmentArr.count, languageName: self.languageName,venueName: "", DepartmentName: "", genderType: "", conatctName: "", isVenueSelect: false , venueTitleName : "" , addressname : "" , cityName : "" , stateName : "" , zipcode: "",startTimeForPicker: Date() , endTimeForPicker: Date())
    ...
    /*    if blockedAppointmentArr.contains(where: { BlockedAppointmentData in
            if BlockedAppointmentData.languageID == 0 {
                print("language id not found at index \(BlockedAppointmentData.rowIndex)")
                return false
            }else if BlockedAppointmentData.venueID == "0" {
            ...
            self.showAlertwithmessage(message: "Please fill all mandatory fields.")
        }*/
    }
    ...
    @IBAction func actionRecurringType(_ sender: UIButton) {
        ...
        self.recurringAppointmentView.isHidden = false
        self.blockedAppointmentView.isHidden = true
    }
    ...
    @IBAction func actionBlockedType(_ sender: UIButton) {
        ...
        self.blockedAppointmentView.isHidden = false
        self.recurringAppointmentView.isHidden = true
    }
    ...
    func createBlockedAppointment(){
        ...
        let selectedText = blockedLanguageTF.text ?? ""
        GetPublicData.sharedInstance.apiGetAllLanguageResponse?.languageData?.forEach({ languageData in
        ...
        if isGenderSelect {
            ...
            self.genderDetail.forEach({ languageData in
                ...
                if blockedGenderTF.text ?? "" == languageData.Value {
                    self.genderID = languageData.Code
        ...
        let startDate = "\(self.blockedAptDateTF.text ?? "") \(self.blockedAptStartTimeTF.text ?? "")"
        let endDate = "\(self.blockedAptDateTF.text ?? "") \(self.blockedEndTimeTF.text ?? "")"
        let requestedOn = self.requestedONTF.text ?? ""
        let location = self.locationTF.text ?? ""
        let textnote = self.specialNotesTF.text ?? ""
        self.jobType = "Onsite Interpretation"
        print("venueID , language ID \(venueID ),\(languageID)")
        if self.blockedAptDateTF.text!.isEmpty {
            self.view.makeToast("Please fill Start Date.",duration: 1, position: .center)
            return
            
        }else if self.blockedAptStartTimeTF.text!.isEmpty {
            self.view.makeToast("Please fill Start Time.",duration: 1, position: .center)
            return
            
        }else if self.blockedEndTimeTF.text!.isEmpty {
...
//
                            ...
                            if let jsonArray = try JSONSerialization.jsonObject(with: data, options : .allowFragments) as? [Dictionary<String,Any>]
                            {
                                ...
                                showLanguageForBlocked()
                                showblockedGenderDropdown()
